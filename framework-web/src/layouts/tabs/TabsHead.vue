<template>
  <div :class="['tabs-head', layout, pageWidth]">
    <a-tabs
      type="editable-card"
      :class="['tabs-container', layout, pageWidth, {'affixed' : affixed, 'fixed-header' : fixedHeader, 'collapsed' : tabsLayout.collapsed}]"
      :active-key="active"
      :hide-add="true"
    >
    ...
    >
      <div class="tabs-page-header-wide">
        <div class="breadcrumb">
          <a-breadcrumb>
            <a-breadcrumb-item v-for="(item, index) in getRouteBreadcrumb()" :key="index">
              <span>{{ item }}</span>
            </a-breadcrumb-item>
          </a-breadcrumb>
        </div>
    ...
    <div v-if="multiPage && affixed" class="virtual-tabs-page-header" />
  </div>
</template>
...
export default {
  ...
  computed: {
    ...mapState('setting', ['layout', 'multiPage', 'pageWidth', 'fixedHeader', 'fixedTabs', 'customTitles']),
    lockTitle() {
  ...
  methods: {
    ...
    getRouteBreadcrumb() {
      ...
      this.pageTitle = breadcrumb[breadcrumb.length - 1]
      return breadcrumb
    }
  }
}
